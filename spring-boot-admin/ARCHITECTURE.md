...
基于 RuoYi 风格的企业级后台管理系统，采用前后端分离架构。
...
└── src/
    ...
    └── views/                           # 页面组件 (20个)
        ...
        └── tool/
            ├── gen/index.vue            # 代码生成器
            ├── swagger/index.vue        # 系统接口 (iframe嵌入)
            └── build/index.vue          # 在线构建器
```
...
| 表 | 用途 | 关键字段 |
|------|------|---------|
| `sys_user` | 用户 | id, username, password(含{noop}前缀), nickname, status |
...
| 表 | 用途 | 记录方式 |
|------|------|---------|
| `sys_login_log` | 登录日志 | TokenService.login() 自动记录 |
| `sys_oper_log` | 操作日志 | AOP 切面自动记录 |
...
dynamicRouteMap 在 `router/index.js` 中定义，包含所有页面组件的懒加载映射：
```js
...
图标使用 Element UI 图标：`<i :class="'el-icon-' + item.icon.toLowerCase()">`。
...
public final Map<Long, List<String>> userTokens;  // userId → token集合
```
...
### 7.1 route/index.js — 路由映射

```js
// 公开路由（无需认证）
constantRoutes = [{ path: '/login', component: Login }]

// 动态路由映射（component路径 → 懒加载函数）
dynamicRouteMap = {
...
10. Axios 响应拦截器 → code === 200 → return data
11. 页面渲染 el-table
```
